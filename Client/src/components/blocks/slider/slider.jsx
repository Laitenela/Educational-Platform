function Slider({ blockName, items }) {
  ...
  const changeOnActiveSliderContainer = (event) => {
    ...
    const activeElements = [document.getElementById(`${activeId}image-slider__slide`), document.getElementById(`${activeId}info-slider__container`)];
    const eventElements = [document.getElementById(`${eventId}image-slider__slide`), document.getElementById(`${eventId}info-slider__container`)];
    if (eventId > activeId) {
    ...
    <div className={`${blockName}__slider-container`}>
      <div className="image-slider">
        {items.map((slide, index) => {
          return (
            <div className={`image-slider__slide${index === activeId ? " active" : ""}`} key={index} id={`${index}image-slider__slide`}>
              <div
                className="image-slider__image"
                id={`${index}image-slider__image`}
                onMouseEnter={(event) => (timer = setTimeout(changeOnActiveSliderContainer, 140, event))}
